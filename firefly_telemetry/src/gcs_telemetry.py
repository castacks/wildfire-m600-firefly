...
class GCSTelemetry:
    def __init__(self):
        self.new_fire_pub = rospy.Publisher(
            "new_fire_bins", Int32MultiArray, queue_size=100
        )
        self.new_no_fire_pub = rospy.Publisher(
            "new_no_fire_bins", Int32MultiArray, queue_size=100
        )
        self.init_to_no_fire_with_pose_pub = rospy.Publisher(
            "init_to_no_fire_with_pose_bins", Pose, queue_size=100
        )
        self.local_pos_ref_pub = rospy.Publisher(
            "local_pos_ref", NavSatFix, queue_size=100
        )
        self.camera_health_gcs_status = rospy.Publisher(
            "camera_health_telem", Bool, queue_size=10
        )
        self.battery_status_gcs = rospy.Publisher(
            "battery_status_telem", Float32, queue_size=10
        )
        self.altitude_status_gcs = rospy.Publisher(
            "altitude_telem", Float32, queue_size=10
        )
        self.temperature_status_gcs = rospy.Publisher(
            "temperature_status_telem", Float32, queue_size=10
        )
            ...
            if msg["seq_num"] == self.nr:
                ...
                while True:
                    if self.nr in self.map_received_buf:
                        map_packet_type, updated_bins_msg = self.map_received_buf.pop(
                            self.nr
                        )
                        if map_packet_type == 0:
                            self.new_fire_pub.publish(updated_bins_msg)
