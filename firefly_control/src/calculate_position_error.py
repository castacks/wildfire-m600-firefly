...
class Node:
    def __init__(self) -> None:
        ...
        self.euclidean_error_pub = rospy.Publisher("euclidean_error", Float32, queue_size=1)

        self.tfBuffer = tf2_ros.Buffer()
        self.listener = tf2_ros.TransformListener(self.tfBuffer)

        self.publish_rate = rospy.get_param('~publish_rate', 10)
        timer = rospy.Timer(rospy.Duration(1.00/self.publish_rate), self.timer_callback)



    def timer_callback(self, event):
        try:
            odom_transform = self.tfBuffer.lookup_transform('uav1/base_link', 'uav1/map', rospy.Time(0.0))
            tracking_point_transform = self.tfBuffer.lookup_transform('uav1/tracking_point', 'uav1/map', rospy.Time(0.0))
        ...
        euclidean_dist = np.sqrt((odom_x - tracking_point_x)**2 + (odom_y - tracking_point_y)**2 + (odom_z - tracking_point_z)**2)
