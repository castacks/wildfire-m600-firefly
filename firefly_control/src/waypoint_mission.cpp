...
ros::Subscriber rtk_position_subscriber;

void gpsPosCallback(const sensor_msgs::NavSatFix::ConstPtr &msg) {
    gps_pos = *msg;
}

void rtkPosCallback(const sensor_msgs::NavSatFix::ConstPtr &msg) {
    rtk_position = *msg;
    if (rtk_position.longitude == 0 || rtk_position.latitude == 0 || rtk_position.altitude == 0)
        ROS_INFO("no rtk signal \n");
    else
        ROS_INFO("rtk tests successfully: longitude:%lf, latitude:%lf,altitude:%lf \n", rtk_position.longitude,
                 rtk_position.latitude, rtk_position.altitude);
...
int main(int argc, char **argv) {
    ...
    rtk_position_subscriber = nh.subscribe<sensor_msgs::NavSatFix>(
            "dji_sdk/rtk_position", 5, &rtkPosCallback);
