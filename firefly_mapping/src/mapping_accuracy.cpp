/********************************************************************************
 * Project FireFly : 2022 MRSD - Team D * Authors: Arjun Chauhan, Kevin Gmelin,
 *Sabrina Shen, Manuj Trehan and Akshay Venkatesh      *
 *                                                                              *
 * MappingAccuracy : System accuracy computation module *
 *                                                                              *
 * Performs computaion of detection and assocaition accuracy of real-time * fire
 *map being generated using pre-loaded ground truth hotspot GPS locations *
 *                                                                              *
 * Created:  11 Apr 2022 *
 ********************************************************************************/
...
map->fire_bin_to_gt: constantly maintained ground truth map for the expected
values of each bin wrt gtfires. used to compute accuracy truth values.
*/

class MappingAccuracy {
 public:
  MappingAccuracy() {
    nh.param<float>("/gcs_mapping/resolution", resolution, 0.5);
    nh.param<float>("/gcs_mapping/min_x", minX, -100.0);
    nh.param<float>("/gcs_mapping/max_x", maxX, 100.0);
    nh.param<float>("/gcs_mapping/min_y", minY, -100.0);
    nh.param<float>("/gcs_mapping/max_y", maxY, 100.0);
    mapWidth = (maxX - minX);
    mapHeight = (maxY - minY);

    init_gts();
    map_sub = nh.subscribe("observed_firemap", 10,
                           &MappingAccuracy::map_callback, this);
    new_fire_sub = nh.subscribe("new_fire_bins", 1000,
                                &MappingAccuracy::new_fire_bins_callback, this);
    new_no_fire_sub =
        nh.subscribe("new_no_fire_bins", 1000,
                     &MappingAccuracy::new_no_fire_bins_callback, this);
  ...
  void map_callback(const nav_msgs::OccupancyGrid& map) { current_map = map; }
  ...
  void new_fire_bins_callback(const std_msgs::Int32MultiArray& msg) {
    // calculate euclidean distance. update associated_gts and fire_bin_to_gt.
    // update accuracies
    int row, col;
    ...
    for (int bin : msg.data) {
      ...
      for (int i = 0; i < gtfire.size();
           ++i) {  // get closest gtfire bin and index
        dist = sqrt(pow((gtfire[i].first - row), 2) +
                    pow((gtfire[i].second - col), 2));
  ...
  void new_no_fire_bins_callback(const std_msgs::Int32MultiArray& msg) {
    ...
    for (int bin : msg.data) {
      ...
      if (fire_bin_to_gt.find(p) !=
          fire_bin_to_gt.end())  // Converting fire bin to no fire bin
      {
...
int main(int argc, char** argv) {
  ...
  while (!ros::param::has("/gcs_mapping/min_x")) continue;
  while (!ros::param::has("/gcs_mapping/min_y")) continue;
  while (!ros::param::has("/gcs_mapping/max_x")) continue;
  while (!ros::param::has("/gcs_mapping/max_y")) continue;
