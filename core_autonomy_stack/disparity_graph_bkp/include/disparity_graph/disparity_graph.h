...
public:
 ...
 bool isStateValidDepth_pose(geometry_msgs::PoseStamped checked_state, double thresh, double& occupancy);
 ...
 void clear_graph(void) { boost::mutex::scoped_lock lock(io_mutex); disp_graph.clear(); first = true; ROS_ERROR_STREAM("<<< DISP GRAPH CLEARED >>>"); };
