...
class State:
    ...
    def print_structure(self):
        print('\t' + self.name)
        for transition, state_or_state_machine in self.transitions.items():
...
class StateMachine:
    ...
    def print_structure(self):
        print(self.name)
        for transition, state_machine in self.transitions.items():
...
class ParallelStateMachine:
    def __init__(self, config_filename):
        ...
        for state_machine_dict in config['state_machines']:
            name = list(state_machine_dict.keys())[0]
        ...
        self.current_state_machine = self.state_machines[list(self.state_machines.keys())[0]]
        
        self.print_structure()
        
    def print_structure(self):
        for state_machine_name, state_machine in self.state_machines.items():
