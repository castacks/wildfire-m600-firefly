...
bool LocalPlanner::initialize()
{
  ...
  ROS_INFO("start init local planner");
  ...
  tracking_point_sub = nh->subscribe("tracking_point", 10, &LocalPlanner::tracking_point_callback, this);
  range_up_sub = nh->subscribe<sensor_msgs::Range>("range_up", 1, &LocalPlanner::range_up_callback, this);
  range_down_sub = nh->subscribe<sensor_msgs::Range>("range_down", 1, &LocalPlanner::range_down_callback, this);
  custom_waypoint_sub = nh->subscribe<geometry_msgs::PoseStamped>("custom_waypoint", 1,
                                                                  &LocalPlanner::custom_waypoint_callback, this);
  ...
  look_past_vis_pub =
      nh->advertise<visualization_msgs::MarkerArray>("look_past", 10);
  ...
  ROS_INFO("done init local planner");
...
bool LocalPlanner::execute()
{
  ...
  {
    if (!got_range_up || !got_range_down)
      return true;

    try
    {
      tf::StampedTransform transform_up, transform_down;
      listener->waitForTransform(gp.get_frame_id(), range_up.header.frame_id, range_up.header.stamp, ros::Duration(0.1));
      listener->lookupTransform(gp.get_frame_id(), range_up.header.frame_id, range_up.header.stamp, transform_up);
      listener->waitForTransform(gp.get_frame_id(), range_down.header.frame_id, range_down.header.stamp, ros::Duration(0.1));
      listener->lookupTransform(gp.get_frame_id(), range_down.header.frame_id, range_down.header.stamp, transform_down);
  ...
  z_setpoint = std::min(range_up_gp_frame.z() - 1.5*robot_radius, std::max(range_down_gp_frame.z() + 1.5*robot_radius, z_setpoint));
  ...
  bool success = tflib::to_frame(listener, look_ahead_position,
                                 look_ahead_odom.header.frame_id, gp.get_frame_id(),
  ...
  bool valid = gp.get_trajectory_distance_at_closest_point(look_ahead_position,
                                                           &trajectory_distance);
  if (valid)
  {
    global_plan_trajectory_distance += trajectory_distance;
    gp = gp.get_subtrajectory_distance(trajectory_distance,
                                       trajectory_distance + 10.);
  ...
  if (!all_in_collision)
  {
    ...
    if (yaw_mode == SMOOTH_YAW && path.waypoints.size() > 0)
    {
      ...
      try
      {
        ...
        listener->waitForTransform(best_traj.get_frame_id(), look_ahead_odom.header.frame_id, look_ahead_odom.header.stamp, ros::Duration(0.1));
        listener->lookupTransform(best_traj.get_frame_id(), look_ahead_odom.header.frame_id, look_ahead_odom.header.stamp, transform);

        transform.setOrigin(tf::Vector3(0, 0, 0)); // only care about rotation
        initial_heading = tf::getYaw(transform * tflib::to_tf(look_ahead_odom.pose.pose.orientation));

        found_initial_heading = true;
      }
      catch (const tf::TransformException &ex)
      {
        ROS_ERROR_STREAM("Transform exception while finding yaw of lookahead: " << ex.what());
      }
      ...
      if (found_initial_heading)
      {
        ...
        // ROS_INFO("-------------------------------------------------");
        // ROS_INFO_STREAM("heading: " << 180./M_PI*tf::getYaw(tflib::to_tf(look_ahead_odom.pose.pose.orientation)));
        for (int i = 1; i < path.waypoints.size(); i++)
        {
          ...
          // ROS_INFO_STREAM(wp_curr.position.y - wp_prev.position.y << " " <<  wp_curr.position.x - wp_prev.position.x);
          double yaw = atan2(wp_curr.position.y - wp_prev.position.y, wp_curr.position.x - wp_prev.position.x);
    ...
    path.header.stamp = ros::Time::now(); // TODO: make this match the time inside get_best_traj
    ROS_INFO_THROTTLE(1, "publishing to traj");
    traj_pub.publish(path);
  }
  else
  {
    ROS_INFO_THROTTLE(1, "all in collision");
  }

  return true;
}

bool LocalPlanner::get_best_trajectory(std::vector<Trajectory> trajectories,
                                       Trajectory global_plan, Trajectory *best_traj)
{
  ROS_INFO_THROTTLE(1, "get bets traj");
  ROS_INFO_STREAM_THROTTLE(1, trajectories.size());
  ...
  */
  ...
  ROS_INFO_STREAM_THROTTLE(1, "0,0: " << values[0][0]);
  ...
  for (int i = 0; i < trajectories.size(); i++)
  {
    ...
    catch (tf::TransformException &ex)
    {
      ...
      ROS_INFO_THROTTLE(1, "failing still with transform exception");
      return true;
    }
    ...
    for (int j = 0; j < traj.waypoint_count(); j++)
    {
      ...
      closest_obstacle_distance = std::min(closest_obstacle_distance,
                                           values[i][j]);
      ...
      bool valid = global_plan_traj_frame.get_closest_point(wp.position(),
                                                            &closest_point, &wp_index, &path_distance);
      double forward_progress_penalty = -forward_progress_penalty_weight * path_distance; //(global_plan_traj_frame.waypoint_count()-wp_index)*0.5;
      ...
      if (valid)
      {
        ...
        average_distance += closest_point.distance(wp.position()) + forward_progress_penalty;
      }
    }
    ...
    if (look_past_distance > 0)
    {
      if (traj.waypoint_count() >= 2)
      {
        ...
        tf::Vector3 closest_point;
        int wp_index;
        bool valid = global_plan_traj_frame.get_closest_point(position,
                                                              &closest_point, &wp_index);
    ...
    double cost = average_distance - obstacle_penalty_weight * std::min(closest_obstacle_distance, obstacle_check_radius);
...
void LocalPlanner::waypoint_callback(geometry_msgs::PointStamped wp)
{
  ...
  for (auto it = waypoint_buffer.rbegin(); it != waypoint_buffer.rend(); it++)
  {
    ...
    if (prev_wps.size() > 1)
    {
      ...
      float a = pow(prev_wp.point.x - curr_wp.point.x, 2) + pow(prev_wp.point.y - curr_wp.point.y, 2);
      float b = pow(prev_wp.point.x - prev2_wp.point.x, 2) + pow(prev_wp.point.y - prev2_wp.point.y, 2);
      float c = pow(prev2_wp.point.x - curr_wp.point.x, 2) + pow(prev2_wp.point.y - curr_wp.point.y, 2);
      float angle = acos((a + b - c) / sqrt(4 * a * b));

      float angle_diff = fabs(atan2(sin(angle - M_PI), cos(angle - M_PI)));

      // ROS_INFO_STREAM("\tdistance: " << distance << " angle: " << angle*180./M_PI << " angle_diff: " << angle_diff*180./M_PI);

      if (distance >= waypoint_spacing_threshold && angle_diff < waypoint_angle_threshold)
      {
        // ROS_INFO_STREAM("\tADDING wp: " << curr_wp.point.x << " " << curr_wp.point.y << " " << curr_wp.point.z);
    ...
    else
    {
      if (prev_wps.size() == 0)
      {
        // ROS_INFO_STREAM("\tADDING 0 wp: " << curr_wp.point.x << " " << curr_wp.point.y << " " << curr_wp.point.z);
        prev_wps.push_back(curr_wp);
        backwards_global_plan.push_back(waypoint);
      }
      else if (prev_wps.size() == 1)
      {
        ...
        {
          // ROS_INFO_STREAM("\tADDING 1 wp: " << curr_wp.point.x << " " << curr_wp.point.y << " " << curr_wp.point.z);
          prev_wps.push_back(curr_wp);
          backwards_global_plan.push_back(waypoint);
        }
      }
  ...
  */
  ...
  if (got_tracking_point)
  {
    ...
    {
      tf::StampedTransform transform;
      listener->waitForTransform(tracking_point_odom.header.frame_id, wp.header.frame_id, wp.header.stamp, ros::Duration(0.1));
      listener->lookupTransform(tracking_point_odom.header.frame_id, wp.header.frame_id, wp.header.stamp, transform);
...
void LocalPlanner::custom_waypoint_callback(geometry_msgs::PoseStamped wp)
{
  ...
  {
    tf::StampedTransform transform;
    listener->waitForTransform(look_ahead_odom.header.frame_id, wp.header.frame_id, wp.header.stamp, ros::Duration(0.1));
    listener->lookupTransform(look_ahead_odom.header.frame_id, wp.header.frame_id, wp.header.stamp, transform);
...
void LocalPlanner::update_waypoint_mode()
{
  ...
  {
    if (global_plan.waypoints.size() < 2)
    ...
    // ROS_INFO_STREAM("elapsed: " << elapsed_time << " / " << distance/custom_waypoint_timeout_factor << " distance: " << distance);
    ...
    if (got_tracking_point)
    {
      ...
      {
        tf::StampedTransform transform;
        listener->waitForTransform(tracking_point_odom.header.frame_id, global_plan.header.frame_id,
                                   global_plan.header.stamp, ros::Duration(0.1));
        listener->lookupTransform(tracking_point_odom.header.frame_id, global_plan.header.frame_id,
                                  global_plan.header.stamp, transform);

        tf::Vector3 tp_position = tflib::to_tf(tracking_point_odom.pose.pose.position);
        tp_position.setZ(0);
        tf::Vector3 wp_position = transform * tflib::to_tf(global_plan.waypoints.back().position);
        wp_position.setZ(0);
