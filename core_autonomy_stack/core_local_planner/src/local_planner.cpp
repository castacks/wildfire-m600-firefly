...
bool LocalPlanner::initialize() {
  ...
  // init subscribers
  global_plan_sub = nh->subscribe("global_plan", 10,
                                  &LocalPlanner::global_plan_callback, this);
  waypoint_sub =
      nh->subscribe("way_point", 10, &LocalPlanner::waypoint_callback, this);
  look_ahead_sub =
      nh->subscribe("look_ahead", 10, &LocalPlanner::look_ahead_callback, this);
  tracking_point_sub = nh->subscribe(
      "tracking_point", 10, &LocalPlanner::tracking_point_callback, this);
  range_up_sub = nh->subscribe<sensor_msgs::Range>(
      "range_up", 1, &LocalPlanner::range_up_callback, this);
  range_down_sub = nh->subscribe<sensor_msgs::Range>(
      "range_down", 1, &LocalPlanner::range_down_callback, this);
  ...
  obst_vis_pub =
      nh->advertise<visualization_msgs::MarkerArray>("obstaccle_vis", 10);
  global_plan_vis_pub = nh->advertise<visualization_msgs::MarkerArray>(
      "local_planner_global_plan_vis", 10);
  look_past_vis_pub =
      nh->advertise<visualization_msgs::MarkerArray>("look_past", 10);
  traj_pub =
      nh->advertise<core_trajectory_msgs::TrajectoryXYZVYaw>("trajectory", 10);
  traj_track_pub = nh->advertise<core_trajectory_msgs::TrajectoryXYZVYaw>(
      "trajectory_track", 10);
  ...
  waypoint_angle_threshold =
      pnh->param("waypoint_angle_threshold", 30.) * M_PI / 180.;

  goal_mode = TRAJECTORY;
  custom_waypoint_timeout_factor =
      pnh->param("custom_waypoint_timeout_factor", 0.3);
  custom_waypoint_distance_threshold =
      pnh->param("custom_waypoint_distance_threshold", 0.5);

  std::string traj_lib_config_filename =
      pnh->param("trajectory_library_config", std::string(""));
  if (traj_lib_config_filename == "") {
    ROS_ERROR_STREAM("Trajectory library config file is invalid: "
  ...
  try {
    ...
    ROS_ERROR("The MapRepresentation plugin failed to load. Error: %s",
              ex.what());
  }
...
bool LocalPlanner::execute() {
  ...
  if (height_mode == FIXED_HEIGHT) {
    ...
    try {
      ...
      tf::Vector3 range_up_gp_frame =
          transform_up * tf::Vector3(range_up.range, 0, 0);
      tf::Vector3 range_down_gp_frame =
          transform_down * tf::Vector3(range_down.range, 0, 0);
      ...
      */
      ...
      double z_setpoint =
          (range_up_gp_frame.z() + range_down_gp_frame.z()) / 2.;
  ...
  tf::Vector3 look_ahead_position =
      tflib::to_tf(look_ahead_odom.pose.pose.position);
  bool success = tflib::to_frame(
      listener, look_ahead_position, look_ahead_odom.header.frame_id,
  ...
  bool valid = gp.get_trajectory_distance_at_closest_point(
      look_ahead_position, &trajectory_distance);
  if (valid) {
  ...
  if (!all_in_collision) {
    core_trajectory_msgs::TrajectoryXYZVYaw path =
        best_traj.get_TrajectoryXYZVYaw();
    ...
    if (yaw_mode == SMOOTH_YAW && path.waypoints.size() > 0) {
      ...
      try {
        ...
        listener->lookupTransform(best_traj.get_frame_id(),
        ...
        initial_heading = tf::getYaw(
            transform * tflib::to_tf(look_ahead_odom.pose.pose.orientation));

        found_initial_heading = true;
      } catch (const tf::TransformException& ex) {
        ROS_ERROR_STREAM("Transform exception while finding yaw of lookahead: "
                         << ex.what());
      }
    ...
    path.header.stamp = ros::Time::now();  // TODO: make this match the time
                                           // inside get_best_traj
  ...
  */
  ...
  for (int i = 0; i < trajectories.size(); i++) {
    ...
    try {
      ...
      ROS_ERROR_STREAM(
          "TransformException in get_best_trajectory: " << ex.what());
      return true;
    }
    ...
    for (int j = 0; j < traj.waypoint_count(); j++) {
      ...
      closest_obstacle_distance =
          std::min(closest_obstacle_distance, values[i][j]);

      // find how far this waypoint is from the global plan path
      tf::Vector3 closest_point;
      int wp_index;
      double path_distance;
      bool valid = global_plan_traj_frame.get_closest_point(
    ...
    if (look_past_distance > 0) {
      if (traj.waypoint_count() >= 2) {
        ...
        tf::Vector3 position =
            curr_wp.position() + look_past_distance * segment;
    ...
    double cost = average_distance -
                  obstacle_penalty_weight * std::min(closest_obstacle_distance,
...
void LocalPlanner::global_plan_callback(
...
void LocalPlanner::waypoint_callback(geometry_msgs::PointStamped wp) {
  ...
  for (auto it = waypoint_buffer.rbegin(); it != waypoint_buffer.rend(); it++) {
    ...
    if (prev_wps.size() > 1) {
      ...
      // ROS_INFO_STREAM("\tdistance: " << distance << " angle: " <<
      // angle*180./M_PI << " angle_diff: " << angle_diff*180./M_PI);

      if (distance >= waypoint_spacing_threshold &&
          angle_diff < waypoint_angle_threshold) {
        // ROS_INFO_STREAM("\tADDING wp: " << curr_wp.point.x << " " <<
      ...
      if (prev_wps.size() == 0) {
        // ROS_INFO_STREAM("\tADDING 0 wp: " << curr_wp.point.x << " " <<
        // curr_wp.point.y << " " << curr_wp.point.z);
        prev_wps.push_back(curr_wp);
        backwards_global_plan.push_back(waypoint);
  ...
  if (got_tracking_point) {
    try {
      ...
      listener->lookupTransform(tracking_point_odom.header.frame_id,
                                wp.header.frame_id, wp.header.stamp, transform);

      tf::Vector3 tp_position =
          tflib::to_tf(tracking_point_odom.pose.pose.position);
      ...
      ROS_ERROR_STREAM(
          "Transform exception in waypoint_callback: " << ex.what());
    }
  }
  ...
  */
  ...
  if(got_tracking_point){
    ...
    try{
      tf::StampedTransform transform;
      listener->waitForTransform(tracking_point_odom.header.frame_id,
  wp.header.frame_id, wp.header.stamp, ros::Duration(0.1));
      listener->lookupTransform(tracking_point_odom.header.frame_id,
  wp.header.frame_id, wp.header.stamp, transform);

      tf::Vector3 tp_position =
  tflib::to_tf(tracking_point_odom.pose.pose.position); tf::Vector3 wp_position
  = transform*tflib::to_tf(wp.point);
...
void LocalPlanner::custom_waypoint_callback(geometry_msgs::PoseStamped wp) {
  ...
  try {
    tf::StampedTransform transform;
    listener->waitForTransform(look_ahead_odom.header.frame_id,
                               wp.header.frame_id, wp.header.stamp,
                               ros::Duration(0.1));
    listener->lookupTransform(look_ahead_odom.header.frame_id,
                              wp.header.frame_id, wp.header.stamp, transform);
    ...
    ROS_ERROR_STREAM(
        "TransformException in custom_waypoint_callback: " << ex.what());
  }
}
  ...
  if (goal_mode == CUSTOM_WAYPOINT) {
    ...
    if (got_tracking_point) {
      try {
        ...
        if (tp_position.distance(wp_position) <
            custom_waypoint_distance_threshold) {
