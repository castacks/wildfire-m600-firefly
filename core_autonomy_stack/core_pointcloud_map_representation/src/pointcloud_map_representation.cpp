...
PointCloudMapRepresentation::PointCloudMapRepresentation()  // ros::NodeHandle*
                                                            // nh)
  ...
  lidar_frame =
      pnh->param("pointcloud_map/lidar_frame", std::string("/lidar_frame"));
  lidar_vertical_fov =
      M_PI / 180. * pnh->param("pointcloud_map/lidar_vertical_fov", 15.0);
  ...
  obstacle_check_radius =
      pnh->param("pointcloud_map/obstacle_check_radius", 2.);
  ...
  cloud_sub = nh->subscribe<sensor_msgs::PointCloud2>(
      "uav1/points", 1, &PointCloudMapRepresentation::cloud_callback, this);
  odom_sub = nh->subscribe<nav_msgs::Odometry>(
      "odometry", 1, &PointCloudMapRepresentation::odom_callback, this);
  range_up_sub = nh->subscribe<sensor_msgs::Range>(
      "range_up", 1, &PointCloudMapRepresentation::range_up_callback, this);
  range_down_sub = nh->subscribe<sensor_msgs::Range>(
      "range_down", 1, &PointCloudMapRepresentation::range_down_callback, this);
  ...
  debug_pub =
  nh->advertise<visualization_msgs::MarkerArray>("/pointcloud_map_debug", 1);
  */
  pose_graph_pub = nh->advertise<visualization_msgs::MarkerArray>(
      "pointcloud_map/pose_graph", 1);
...
std::vector<std::vector<double> > PointCloudMapRepresentation::get_values(
    std::vector<std::vector<geometry_msgs::PointStamped> >
        trajectories) {  // std::vector<core_trajectory_msgs::TrajectoryXYZVYaw>
                         // trajectories){
  ...
  for (int i = 0; i < trajectories.size(); i++) {
    ...
    for (int j = 0; j < trajectories[i].size(); j++) {
      ...
      // listener->waitForTransform(target_frame, trajectory.header.frame_id,
      // trajectory.header.stamp, ros::Duration(0.1));
      // listener->lookupTransform(target_frame, trajectory.header.frame_id,
      // trajectory.header.stamp, transform);
      listener->waitForTransform(target_frame, point_stamped.header.frame_id,
                                 point_stamped.header.stamp,
                                 ros::Duration(0.1));
      listener->lookupTransform(target_frame, point_stamped.header.frame_id,
                                point_stamped.header.stamp, transform);

      // tf::Vector3 position =
      // transform*tflib::to_tf(trajectory.waypoints[j].position);
      ...
      for (int k = 0; k < nodes.size(); k++) {
        tf::Vector3 position_cloud_frame =
            nodes[k].target_to_lidar_frame_transform * position;
        double angle = position_cloud_frame.angle(tf::Vector3(
            position_cloud_frame.x(), position_cloud_frame.y(),
            0));  // atan2(position_cloud_frame.z(), position_cloud_frame.x());
        double distance = position_cloud_frame.length();
        // ROS_INFO_STREAM("angle: " << 180./M_PI*angle << " lidar v fov: " <<
        // 180./M_PI*lidar_vertical_fov/2. << " distance: " << distance);
...
void PointCloudMapRepresentation::clear() {
  for (int i = 0; i < 20; i++) ROS_INFO("MAP CLEARED");
  node_buffer.clear();
}

double PointCloudMapRepresentation::distance_to_obstacle(
    geometry_msgs::PoseStamped pose, tf::Vector3 direction) {
  try {
    bool seen = false;

    tf::StampedTransform transform;
    listener->waitForTransform(target_frame, pose.header.frame_id,
                               pose.header.stamp, ros::Duration(0.1));
    listener->lookupTransform(target_frame, pose.header.frame_id,
    ...
    for (int i = 0; i < nodes.size(); i++) {
      tf::Vector3 position_cloud_frame =
          nodes[i].target_to_lidar_frame_transform * position;
      double angle = position_cloud_frame.angle(tf::Vector3(
          position_cloud_frame.x(), position_cloud_frame.y(),
          0));  // atan2(position_cloud_frame.z(), position_cloud_frame.x());
      double distance = position_cloud_frame.length();
      ...
      if(i == 0){
        if(nodes[i].range_up_valid){
          double xy_distance = sqrt(pow(nodes[i].range_point_up.x() - point.x,
      2) + pow(nodes[i].range_point_up.y() - point.y, 2)); if(point.z >=
      nodes[i].range_point_up.z()){ if(xy_distance <= nodes[i].up_radius)
              min_distance = 0;
          }
          else if(xy_distance <= nodes[i].up_radius){
            min_distance = std::min(min_distance,
      (double)(nodes[i].range_point_up.z() - point.z));
          }
        }
      }
      */
    ...
    debug_point.z = position.z();
    debug_point.r =
        seen ? 0 : 255;  //(1.0 - std::min(min_distance, 10.0)/10.0)*255;
    ...
    ROS_ERROR_STREAM(
        "Transform Exception in distance_to_obstacle while looking up tf from "
...
void PointCloudMapRepresentation::publish_debug() {
  ...
  if (nodes.size() > 0) {
    ...
    for (int i = 0; i < nodes.size(); i++) {
      ...
      pose.scale.y = 0.1;
      pose.scale.z = 0.1;
      float age = node_decay_time > 0
                      ? (now - curr_odom.header.stamp).toSec() / node_decay_time
                      : 0.f;
      pose.color.r = age;
      pose.color.g = 1 - age;
      pose.color.b = 0;
      ...
      if (i == 0) {
        ...
        if (nodes[i].range_up_valid) marker_array.markers.push_back(circle_up);
...
void PointCloudMapRepresentation::cloud_callback(
  ...
  try {
    ...
    listener->waitForTransform(target_frame, lidar_frame, cloud.header.stamp,
                               ros::Duration(0.1));
    listener->lookupTransform(target_frame, lidar_frame, cloud.header.stamp,
                              lidar_to_target_frame_transform);
    // ROS_INFO_STREAM("transformPointCloud: target_frame " << target_frame << "
    // cloud frame " << cloud.header.frame_id);
    pcl_ros::transformPointCloud(target_frame, cloud, cloud_target_frame,
                                 *listener);
    pcl::PointCloud<pcl::PointXYZ>::Ptr current_cloud(
        new pcl::PointCloud<pcl::PointXYZ>());
    pcl::fromROSMsg(cloud_target_frame, *current_cloud);

    if (current_cloud->points.size() == 0) return;
    ...
    current_node.target_to_lidar_frame_transform =
        lidar_to_target_frame_transform.inverse();

    current_node.range_point_up = range_point_up;
    current_node.range_up_valid =
        range_up_valid &&
        (fabs((range_up_time - odom.header.stamp).toSec()) < 0.5);
    current_node.up_radius = up_radius;

    current_node.range_point_down = range_point_down;
    current_node.range_down_valid =
        range_down_valid &&
        (fabs((range_down_time - odom.header.stamp).toSec()) < 0.5);
  ...
  if (node_buffer.size() > 0) {
    double distance =
        tflib::to_tf(odom.pose.pose.position)
            .distance(tflib::to_tf(
                node_buffer[node_buffer.size() - 1].odom.pose.pose.position));
    if (distance >= node_spacing) should_add_node = true;
  } else
...
void PointCloudMapRepresentation::range_up_callback(sensor_msgs::Range range) {
  ...
  range_up_valid = get_point_in_frame(target_frame, range.header.frame_id,
  range.header.stamp, point_range_frame, &range_point_up); range_up_time =
  range.header.stamp;

  tf::Vector3 point_lidar_frame;
  bool lidar_point_valid = get_point_in_frame(lidar_frame,
  range.header.frame_id, range.header.stamp, point_range_frame,
  &point_lidar_frame); if(lidar_point_valid){ up_radius =
  point_lidar_frame.z()/tan(0.5*lidar_vertical_fov);
  }
  else
    range_up_valid = false;
  */
...
void PointCloudMapRepresentation::range_down_callback(
    sensor_msgs::Range range) {
  /*
  tf::Vector3 point_range_frame(range.range, 0, 0);
  range_down_valid = get_point_in_frame(target_frame, range.header.frame_id,
  range.header.stamp, point_range_frame, &range_point_down); range_down_time =
  range.header.stamp;

  tf::Vector3 point_lidar_frame;
  bool lidar_point_valid = get_point_in_frame(lidar_frame,
  range.header.frame_id, range.header.stamp, point_range_frame,
  &point_lidar_frame); if(lidar_point_valid){ down_radius =
  -point_lidar_frame.z()/tan(0.5*lidar_vertical_fov);
  }
  else
    range_up_valid = false;
...
bool PointCloudMapRepresentation::get_point_in_frame(
    std::string target_frame, std::string frame_id, ros::Time time,
    tf::Vector3 point_range_frame, tf::Vector3* point_target_frame) {
  try {
    tf::StampedTransform transform;
    listener->waitForTransform(target_frame, frame_id, time,
                               ros::Duration(0.1));
...
std::vector<PointCloudNode> PointCloudMapRepresentation::get_all_nodes() {
  ...
  for (int i = 0; i < node_buffer.size(); i++)
    if (node_decay_time <= 0.f ||
        (now - node_buffer[i].odom.header.stamp).toSec() <= node_decay_time)
      nodes.push_back(node_buffer[i]);
  return nodes;
}

// UNTESTED
tf::Vector3 PointCloudMapRepresentation::distance_vector_to_obstacle(
    geometry_msgs::PoseStamped pose, tf::Vector3 direction) {
  try {
    tf::StampedTransform transform;
    listener->waitForTransform(target_frame, pose.header.frame_id,
    ...
    for (int i = 0; i < nodes.size(); i++) {
      ...
      std::vector<int> indices;
      std::vector<float> squared_distances;
      if (kdtree == NULL) return tf::Vector3(0, 0, 0);
...
/*
double
PointCloudMapRepresentation::distance_to_obstacle(geometry_msgs::PoseStamped
pose, tf::Vector3 direction){ try{ tf::StampedTransform transform;
    listener->waitForTransform(target_frame, pose.header.frame_id,
pose.header.stamp, ros::Duration(0.1)); listener->lookupTransform(target_frame,
pose.header.frame_id, pose.header.stamp, transform);
...
dist*direction_vectors[m];//tf::Transform(q_curr)*(dist*direction);
          ...
          if(collision){
            closest_obstacle_distance = std::min(dist,
closest_obstacle_distance); c = red;
          }
          else
            c = green;
    ...
    ROS_ERROR_STREAM("Transform Exception in distance_to_obstacle while looking
up tf from " << pose.header.frame_id << " to " << target_frame << ": " <<
ex.what());
  }
...
void PointCloudMapRepresentation::publish_debug(){
  ...
  // publish pose graph
  std::vector<PointCloudNode> nodes = get_all_nodes();
  pcl::PointCloud<pcl::PointXYZ>::Ptr all_clouds(new
pcl::PointCloud<pcl::PointXYZ>()); if(nodes.size() > 0){
    visualization_msgs::MarkerArray marker_array;
    ros::Time now = ros::Time::now();

    for(int i = 0; i < nodes.size(); i++){
      nav_msgs::Odometry curr_odom = nodes[i].odom;
      all_clouds->points.insert(all_clouds->points.begin(),
nodes[i].cloud->points.begin(), nodes[i].cloud->points.end());
